...
export const AdminKitchenView = ({ orders, handleOrderAction }) => {
  return (
    <div className="h-full flex flex-col animate-fade-in">
      <div className="flex lg:grid lg:grid-cols-4 gap-4 md:gap-6 h-full overflow-x-auto pb-4 snap-x snap-mandatory">
              ...
              orders.filter(o => o.status === 'PENDIENTE').map(order => (
                <div key={order.id} className="bg-[#1A1A1E] border border-gray-700/50 p-4 md:p-5 rounded-xl shadow-lg border-l-4 border-l-red-500">
                  ...
                  <ul className="text-base text-gray-300 space-y-2 mb-5 font-medium">
                    {order.items.map((item, idx) => <li key={`${order.id}-${idx}`} className="flex items-start"><span className="text-red-500 mr-2 mt-0.5">•</span> {item}</li>)}
                  </ul>
            ...
            {orders.filter(o => o.status === 'PREPARANDO').length === 0 ? (
              ...
              orders.filter(o => o.status === 'PREPARANDO').map(order => (
                <div key={order.id} className={`bg-[#1A1A1E] border border-gray-700/50 p-4 md:p-5 rounded-xl shadow-lg border-l-4 ${order.time > 15 ? 'border-l-red-500 shadow-[0_0_20px_rgba(239,68,68,0.15)]' : 'border-l-indigo-500'}`}>
                  ...
                  <ul className="text-base text-gray-300 space-y-2 mb-5 font-medium">
                    {order.items.map((item, idx) => <li key={`${order.id}-${idx}`} className="flex items-start"><span className="text-indigo-500 mr-2 mt-0.5">•</span> {item}</li>)}
                  </ul>
            ...
            {orders.filter(o => o.status === 'LISTO').length === 0 ? (
              ...
              orders.filter(o => o.status === 'LISTO').map(order => (
                <div key={order.id} className="bg-[#1A1A1E] border border-gray-700/50 p-4 md:p-5 rounded-xl shadow-lg border-l-4 border-l-green-500">
                  ...
                  <ul className="text-base text-gray-300 space-y-2 mb-5 font-medium">
                    {order.items.map((item, idx) => <li key={`${order.id}-${idx}`} className="flex items-start"><span className="text-green-500 mr-2 mt-0.5">•</span> {item}</li>)}
                  </ul>
                  <button onClick={() => handleOrderAction(order.id, 'ENTREGAR')} className="w-full py-3 bg-green-500/10 text-green-400 text-sm font-bold rounded-lg border border-green-500/20 hover:bg-green-500 hover:text-black transition-colors">
                    {order.table?.includes('Delivery') ? 'ENVIAR A DESPACHO' : 'ENTREGAR A MESA'}
                  </button>
                </div>
              ))
            )}
        ...
        <div className="min-w-[85vw] md:min-w-[350px] lg:min-w-0 bg-[#121214]/50 border border-gray-800/60 rounded-2xl p-4 flex flex-col opacity-60 snap-center">
          <h3 className="text-sm font-bold text-gray-400 mb-4 flex items-center justify-between">
            <span>HISTORIAL / ENTREGADOS</span>
            <span className="bg-gray-800 text-gray-300 px-2 py-0.5 rounded text-xs">{orders.filter(o=>o.status==='ENTREGADO').length}</span>
          </h3>
          <div className="space-y-4 overflow-y-auto custom-scrollbar flex-1 pr-1 flex flex-col">
            {orders.filter(o => o.status === 'ENTREGADO').length === 0 ? (
              <div className="flex-1 flex items-center justify-center text-gray-600 text-sm font-medium border-2 border-dashed border-gray-800 rounded-xl p-8 text-center min-h-[150px] w-full mt-2">
                No hay pedidos en el registro histórico
              </div>
            ) : (
              [...orders].filter(o => o.status === 'ENTREGADO').reverse().slice(0, 30).map(order => (
                <div key={order.id} className="bg-[#1A1A1E]/50 border border-gray-800 p-4 md:p-5 rounded-xl shadow-lg">
                  <div className="flex justify-between items-start mb-4">
                    <span className="font-black text-xl text-gray-400 tracking-tight">{order.table}</span>
                    <span className="text-sm font-bold text-gray-500 bg-gray-800/50 px-2 py-1 rounded-lg">
                      Entregado
                    </span>
                  </div>
                  <ul className="text-sm text-gray-500 space-y-1 font-medium">
                    {order.items.map((item, idx) => <li key={`${order.id}-${idx}`} className="flex items-start"><span className="text-gray-600 mr-2 mt-0.5">-</span> {item}</li>)}
                  </ul>
                </div>
              ))
            )}
